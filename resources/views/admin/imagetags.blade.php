...
@include("admin-layouts.menuAdmin", array("link" => "imageposts", "has_sublink" => 1, "sublink" => "imagetags"))
...
<nav id="right-side-menu" class="side-menu full-height">
	...
	<ul class="root-menu">
		<li><h4>Filter</h4></li>
        <li class="has-pd">
        	{!! Form::open(array("route" => "adminImageTags", "method" => "get")) !!}
				{!! Form::label("q", "Search") !!}
...
<div id="admin-imagetags" class="container">
    ...
    @if (Input::get("q") != null)
		<div class="small-medium-align-center hide-for-large-up">
			...
			@endif
			<p>({!! HTML::linkRoute("adminImageTags", "Reset filter") !!})</p>
			<br />
		</div>
	@endif
	                ...
	                <tr>
	                    <td class="c-show-for-small-only">
	                    	...
	                    	<span>
	                    		@if (!$tag->is_featured)
	                    			@if (Auth::user()->role == "admin")
										<a href="{!! URL::route('adminImageTagTogglefeatured', array($tag->id)) !!}" title="Set Featured">
											<span class="tf-icon fa fa-star"></span>
										</a>
									@endif
								@else
									@if (Auth::user()->role == "admin")
										<a href="{!! URL::route('adminImageTagTogglefeatured', array($tag->id)) !!}" title="Unset Featured">
	                        ...
	                        <span>
	                        	<b>Image posts</b> : 
	                        	{!! HTML::linkRoute("adminImagePosts", $tag->posts()->count()." posts", array("tag" => $tag->name)) !!}
                        	</span>
	                        <br /><br />
	                        @if (Auth::user()->role == "admin")
		                        <span>
		                        	...
		                        	<div id="delete-small-modal-id-{!! $tag->id !!}" class="reveal-modal tiny" data-reveal>
		                        		...
		                        		<p>"{!! $tag->name !!}"</p>
										{!! Form::open(array("route" => array("adminImageTagDelete", $tag->id), 'method' => 'delete')) !!}
